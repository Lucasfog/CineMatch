...
class _DetailsScreenState extends State<DetailsScreen> {
  ...
  Future<void> _checkIfFavorite() async {
    ...
    if (response.statusCode == 200) {
      // Decodifique o corpo da resposta para verificar o valor de isFavorite
      final Map<String, dynamic> responseData = json.decode(response.body);

      setState(() {
        _isFavorite =
            responseData['isFavorite'] ?? false; // Verifique se o campo existe
      });
    } else {
      setState(() {
        _isFavorite =
            false; // Se não for 200, assume que não está nos favoritos
      });
    }
  }
